  ...
  return (
    <main className="min-h-screen bg-[#0a0e1a] p-8">
      <section className="mx-auto max-w-6xl">
        ...
        <p className="mt-2 text-slate-400">Create workouts and assign drills to players.</p>
        ...
        <div className="mt-8 rounded-xl bg-white/4 border border-white/8 p-6">
          ...
          <form action={createWorkout} className="mt-4 space-y-4">
            ...
            <div>
              ...
              {players.length === 0 ? (
                <p className="text-sm text-slate-500">No players yet — add some first.</p>
              ) : (
                <div className="grid gap-3 md:grid-cols-3">
                  {players.map((player) => (
                    <label key={player.id} className="flex items-center gap-3 rounded-lg border border-white/8 p-3 cursor-pointer hover:border-white/15 transition-colors">
                      <input type="checkbox" name="playerIds" value={player.id} className="accent-orange-500" />
                      <span className="text-sm text-slate-200">
                        {player.firstName} {player.lastName}
                        {player.position && <span className="ml-1 text-slate-500">· {player.position}</span>}
                      </span>
                    </label>
                  ))}
                </div>
        ...
        <div className="mt-8 rounded-xl bg-white/4 border border-white/8 p-6">
          <h2 className="text-xl font-semibold text-slate-100">
            Workout History <span className="text-sm font-normal text-slate-500">({workouts.length})</span>
          </h2>
