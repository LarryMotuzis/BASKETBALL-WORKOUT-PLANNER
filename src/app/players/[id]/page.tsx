...
export default async function PlayerProfilePage({
  ...
  const workouts = await prisma.workout.findMany({
    where: {
      userId,
      workoutPlayers: { some: { playerId: id } },
    },
    include: {
      workoutDrills: { include: { drill: true } },
    },
  ...
  for (const w of workouts) {
    const key = `${w.workoutDate.getFullYear()}-${String(
      w.workoutDate.getMonth() + 1
    ).padStart(2, "0")}`;
    const slot = monthLabels.find((m) => m.key === key);
    if (slot) slot.count++;
  }

  const workoutsChartData = monthLabels.map(({ label, count }) => ({
    month: label,
    count,
  }));
  ...
  const totalDrillReps = workouts.reduce(
    (sum, w) => sum + w.workoutDrills.length,
    0
  );
      ...
      <section className="mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div
              key={stat.label}
              className="rounded-xl bg-white/4 border border-white/8 p-5"
            >
              <p
          ...
          {workouts.length === 0 ? (
            ...
            <div className="space-y-3">
              {workouts.map((workout) => (
                <div
                  key={workout.id}
                  className="flex items-start justify-between rounded-lg border border-white/8 p-4 gap-4"
                >
                  <div>
                    <p className="font-medium text-slate-100">{workout.title}</p>
                    <p className="text-xs text-slate-500 mt-0.5">
                      {workout.workoutDate.toLocaleDateString("en-US", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                      })}
                    ...
                    {workout.workoutDrills.length > 0 && (
                      <p className="text-xs text-slate-500 mt-1">
                        {workout.workoutDrills
                          .map((wd) => wd.drill.name)
                          .join(" · ")}
                      </p>
                    )}
                  </div>
